...
type DefaultShapesType = 'linear' | 'radial' | 'conic'
    ...
    rules: [
      [
        ...
        ([, type = 'linear', precision, mode, body], { theme }) => {
          ...
          if (stops != null) {
            return {
              '--un-gradient-shape': defaultShapes[type as DefaultShapesType],
...
function resolveStops(steps: number, mode: chroma.InterpolationMode, body: string, theme: any) {
  const cs: any[] = []
  while (true) {
  ...
  for (let i = 0; i < s; i++) {
    ...
    const r = resolveChroma(last ? steps : steps + 1, mode, cs[i].color, cs[i + 1].color)
      .map(([c, p]) => [parseColor(c as string, theme)?.cssColor?.components?.join(','), Math.floor(i * 100 / s + (p as number) / s)])
      .map(([c, p]) => `rgb(${c}) ${p}%`)
...
function resolveChroma(steps: number, mode: chroma.InterpolationMode, start: string, end: string) {
