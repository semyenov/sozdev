...
export NPM_GLOBAL=${2:-"/usr/local/share/npm-global"}
export NODE_VERSION=${3:-"lts/*"}
USERNAME=${4:-"node"}
UPDATE_RC=${5:-"true"}
...
if [ "\$(stat -c '%U' \$NPM_GLOBAL)" != "${USERNAME}" ]; then
    if [ "\$(id -u)" -eq 0 ] || type sudo > /dev/null 2>&1; then
        echo "Fixing permissions of \"\$NPM_GLOBAL\"..."
        sudoIf chown -R ${USERNAME}:root \$NPM_GLOBAL
    else
        echo "Warning: NPM GLOBAL directory is not owned by ${USERNAME} and sudo is not installed. Unable to correct permissions."
    fi
fi
